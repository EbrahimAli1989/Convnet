...
def main():
    ...
    root_folder = os.path.join(ConFig.savedir, "{}_{}_nocolor{}_drop{}_{}".format(
        ConFig.model, ConFig.npoints, ConFig.nocolor, ConFig.drop, time_string))

    # create the filelits (train / val) according to area
    print("Create filelist...", end="")
    train_dir = os.path.join(ConFig.rootdir, "train_pointclouds")
    filelist_train = [dataset for dataset in os.listdir(train_dir)]
    test_dir = os.path.join(ConFig.rootdir, "test_pointclouds")
    ...
    Dropv={}
    Dropv['drop']=ConFig.drop
    net = get_model(ConFig.model, input_channels=1, output_channels=N_CLASSES, args=Dropv)
    if ConFig.test:
        net.load_state_dict(torch.load(os.path.join(ConFig.savedir, "state_dict.pth")))
    #net.cuda()
    ...
    if not ConFig.test:
        ...
        ds = PartDataset(filelist_train, train_dir,
                         training=True, block_size=ConFig.block_size,
                         iteration_number=ConFig.batch_size * ConFig.iter,
                         npoints=ConFig.npoints,
                         nocolor=ConFig.nocolor)
        train_loader = torch.utils.data.DataLoader(ds, batch_size=ConFig.batch_size, shuffle=True,
        ...
        for epoch in range(ConFig.epochs):
            ...
            for pts, features, seg in t:
                ...
                output_np = np.argmax(outputs.detach().numpy(), axis=2).copy()
        ...
        for filename in filelist_test:
            ...
            os.makedirs(os.path.join(ConFig.savedir, "results"), exist_ok=True)

            # saving labels
            save_fname = os.path.join(ConFig.savedir, "results", filename)
            scores = scores.argmax(1)
            np.savetxt(save_fname, scores, fmt='%d')

            if ConFig.savepts:
                save_fname = os.path.join(ConFig.savedir, "results", f"{filename}_pts.txt")
