...
# we remove Paris.ply becuase memory issue
train_filenames = ["Lille1_1.ply", "Lille1_2.ply", "Lille2.ply" ]
test_filenames = ["ajaccio_2.ply", "ajaccio_57.ply", "dijon_9.ply"]

if ConFig.test:
    filenames = test_filenames
    save_dir = os.path.join(ConFig.destdir, "test_pointclouds")
else:
    filenames = train_filenames
    save_dir = os.path.join(ConFig.destdir, "train_pointclouds")
os.makedirs(save_dir, exist_ok=True)

for filename in filenames:
    if ConFig.test:
        fname = os.path.join(ConFig.rootdir, "test_10_classes", filename)
    else:
        fname = os.path.join(ConFig.rootdir, "training_10_classes", filename)
    print(fname)
    ...
    if ConFig.test:
        ...
        hist, edges = np.histogram(pts_new, pts_new.shape[0] // 2500000 - 2,
